...
export function extractPreamble(dom: jsdom.JSDOM): string {
  const paragraph = dom.window.document.querySelector(".date ~ p");
  if (!paragraph) throw new Error("result is empty");

  const result = extractTextBeforeBr(paragraph).trim();
  if (!result) throw new Error("result is empty");
  return result;
}

function extractTextBeforeBr(element: Element): string {
  let result = "";
