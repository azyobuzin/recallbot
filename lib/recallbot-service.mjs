...
import { DefaultExtractRecallDetailsService } from "./extract-recall-details-service.mjs";
import {
...
import { DefaultReportErrorService } from "./report-error-service.mjs";
import { DefaultTootService, TootProcedure } from "./toot-service.mjs";
 ...
 * @property {import("../types/services.ts").ExtractRecallDetailsService} extractRecallDetailsService
 ...
 * @property {import("../types/services.ts").ReportErrorService} reportErrorService
 */
...
export class DefaultRecallbotService {
  ...
  static createDefault() {
    return new DefaultRecallbotService({
      downloadResourceService: new DefaultDownloadResourceService(),
      postedUrlsRepository: DefaultPostedUrlsRepository.createDefault(),
      extractRecallDetailsService:
        DefaultExtractRecallDetailsService.createDefault(),
      pdfToImagesService: new DefaultPdfToImagesService(),
      tootService: DefaultTootService.createDefault(),
      reportErrorService: DefaultReportErrorService.createDefault(),
    });
  }
  ...
  async execute() {
    for (const feedItem of await this._getTargetFeedItems()) {
      try {
        ...
        console.error(e);
        await this._deps.reportErrorService.reportError(e);
      }
    }
  }
  ...
  async _post(feedItem) {
    ...
    const recallDetails =
      await this._deps.extractRecallDetailsService.extractRecallDetailsFromPdf(
        recallListPdfBuf,
      );
...
${feedItem.link}`;
    ...
    if (illustrationPdfUrl) {
      ...
      for (const img of images)
        tootPrecedure.uploadMedia(img, "改善箇所説明図");
    }
